      ...
      <div className="fixed inset-0 z-10 overflow-hidden">
        <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
          ...
          >
            ...
            {isUser? ( <>
              <div className={`${state.theme==='light' ? 'text-black' : ''}`} >名称: {userInfo?.nickname||''}</div>
              <div className={`${state.theme==='light' ? 'text-black' : ''}`} >登录账号: {userInfo?.name||''}</div>
              <div className={`${state.theme==='light' ? 'text-black' : ''}`}> 手机号: {userInfo?.phone||''}</div>
              <div className={`${state.theme==='light' ? 'text-black' : ''}`}> 邮箱: {userInfo?.email||''}</div>
